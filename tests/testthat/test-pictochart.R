...
test_that("Label alignment works", {
    expect_error( PictoChart(x3/1000, ppl.filled, K=ceiling(x3/1000),
                ...
                label.right=rownames(x3), label.right.size=5), NA)
})
...
test_that("Accepts multiple background images", {
    expect_error ( PictoChart(self.consumed, K=ceiling(K), variable.image = ppl.filled,
                 base.image=c(ppl.red,ppl.grey,ppl.grey,ppl.red,ppl.red),
                 icon.fixedsize = T,
                 label.left=names(self.consumed),
                 label.right=extype, label.right.color=excol), NA )
})
...
test_that("Accepts varying number of rows/columns of icons", {
    expect_error( PictoChart(tot.water, drop.pic, icon.fixedsize = T,
                 icon.nrow=nmonths, row.height=nmonths*30, icon.ncol=tot.water/nmonths,
                 pad.icon.row=0, pad.icon.col=0, pad.row=20, show.legend=T), NA)
})
...
test_that("Accepts multiple image URLS", {
    expect_error( PictoChart(x6/10, rep(c(cow,pig,chicken,sheep), each=nrow(x6)), show.lines=T,
                 label.top.halign="left", show.legend=T, legend.text="10kg per capita"), NA)
})
